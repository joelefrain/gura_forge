...
from config.config import KM_TO_METERS
...
class FaultIdentifier:
    ...
    def __init__(self, shapefile, author=None, map_name=None, rev_date=None):
        ...
        self.project_to_meters = pyproj.Transformer.from_crs(
            self.wgs84, self.local_crs, always_xy=True).transform
        self.project_to_wgs84 = pyproj.Transformer.from_crs(
            self.local_crs, self.wgs84, always_xy=True).transform
    ...
    def create_circle(self, longitude, latitude, radius, resolution=100):
        ...
        circle_m = center_m.buffer(
            radius * KM_TO_METERS, resolution=resolution)  # Radio en metros
    ...
    def circle_params(self):
        ...
        if self._circle_params is not None:
            df = pd.DataFrame([self._circle_params], columns=['Longitude', 'Latitude', 'Radius'])
            return df
        else:
    ...
    def columns_shp(self):
        ...
        return pd.DataFrame({
            'Column Name': self.map_gdf.columns,
            'Data Type': self.map_gdf.dtypes
        })
    ...
    def metadata(self):
        ...
        return pd.DataFrame({
            ...
            'Value': [self.author or 'No disponible',
